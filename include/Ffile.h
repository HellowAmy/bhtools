...
// 跨平台处理函数
    ...
    namespace bhtools_platform {
        ...
        // 判断文件夹存在
        static bool is_exist_dir(const std::string &path)
        {
        ...
        // 判断为文件
        static bool is_file_type(const std::string &path)
        {
            struct stat st;
            stat(path.c_str(),&st);
        ...
        // 判断为目录
        static bool is_dir_type(const std::string &path)
        {
            struct stat st;
            stat(path.c_str(),&st);
        ...
        // 创建单层目录
        static bool create_dir(const std::string &dir,int mode)
        { return !dir.empty() && !is_exist_dir(dir) && (mkdir(dir.c_str(), mode) != 0); }

        // 创建多层目录
        static bool make_dir(const std::string &dir,int mode)
        {
            size_t pos = 0;
            std::string level;
            ...
            if(create_dir(level,mode)) { return false; }
            return true;
        }
...
{
    // 判断存在-文件
    static bool is_exist_file(const std::string &path)
    ...
    // 创建多层目录
    static bool make_dir(const std::string &dir,int mode = 0755)
    ...
    static std::string get_name(const std::string &file)
    {

    }

    static std::string get_basename(const std::string &file)
    {
