            ...
            while((entry = readdir(dir)) != nullptr) {
                if(std::string(entry->d_name) != "." && std::string(entry->d_name) != "..") {
                    std::string path_name =
                        bhtools_platform::path_merge(path, std::string(entry->d_name));
        ...
        if(hfile != INVALID_HANDLE_VALUE) {
            ...
            do {
                if(std::string(fdata.cFileName) != "." && std::string(fdata.cFileName) != "..") {
                    std::string path_name =
                        bhtools_platform::path_merge(path, std::string(fdata.cFileName));
                    if(is_dir_type(path_name)) {
                        level.push_back(path_name);
                    }
                    else {
...
struct Ffsys
{
    ...
    // 获取当前路径
    inline static std::string get_dir() { return bhtools_platform::get_dir(); }

    // 切换到目标路径
    inline static bool set_dir(const std::string &path) { return bhtools_platform::set_dir(path); }
