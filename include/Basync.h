...
public:
    ...
    // 开始任务
    Basync &next(std::function<bool(Targs *)> fn = nullptr)
    {
        if(fn) {
            _ls_next.push_back(fn);
        }
        return *this;
    };

    // 完成任务回调
    Basync &succee(std::function<void(Targs *)> fn = nullptr)
    {
        _fn_succee = fn;
        return *this;
    };
    ...
    // 完成任务回调
    Basync &error(std::function<void(Targs *)> fn = nullptr)
    {
        _fn_error = fn;
        return *this;
    };
    ...
    bool run(Targs *any = nullptr, bool async = true)
    {
        ...
        if(async) {
            _arg = any;
            quit_th();
            _th = std::make_shared<std::thread>(&Basync::exec_task, this);
        }
        else {
            _arg = any;
            exec_task();
