...
namespace platform {
    ...
    // 删除文件
    inline static bool remove_file(const std::string &file) { return remove(file.c_str()) == 0; }
    ...
    // 获取文件与目录列表
    inline static bool get_dir_info(const std::string &path, std::vector<std::string> &files,
                                    std::vector<std::string> &dirs, bool recursion)
    {
        DIR *dir = opendir(path.c_str());
        if(dir) {
            struct dirent *entry;
            ...
            while((entry = readdir(dir)) != nullptr) {
                if(std::string(entry->d_name) != "." && std::string(entry->d_name) != "..") {
                    std::string path_name = platform::path_merge(path, std::string(entry->d_name));
    ...
    // 删除文件
    inline static bool rm_file(const std::string &path) { return remove(path.c_str()) == 0; }

    // 删除空目录
    inline static bool rm_dir(const std::string &path) { return rmdir(path.c_str()) == 0; }
...
namespace platform {
    ...
    // 获取文件与目录列表
    inline static bool get_dir_info(std::string path, std::vector<std::string> &files,
                                    std::vector<std::string> &dirs, bool recursion)
    {
            ...
            do {
                if(std::string(fdata.cFileName) != "." && std::string(fdata.cFileName) != "..") {
                    std::string path_name =
                        platform::path_merge(path, std::string(fdata.cFileName));
                    if(is_dir_type(path_name)) {
                        level.push_back(path_name);
                    }
                    else {
...
public:
    ...
    // 获取当前路径
    inline static std::string get_dir() { return platform::get_dir(); }

    // 切换到目标路径
    inline static bool set_dir(const std::string &path) { return platform::set_dir(path); }

    // 替换为当前平台路径
    inline static std::string platform_path(std::string path)
    {
        std::string sp = platform::file_splitter();
    ...
    // 判断为目录
    inline static bool is_dir_type(const std::string &path) { return platform::is_dir_type(path); }

    // 删除文件
    inline static bool remove_file(const std::string &file) { return platform::remove_file(file); }
    ...
    // 获取文件和目录
    inline static std::tuple<std::vector<std::string>, std::vector<std::string>> get_files_and_dirs(
        const std::string &path, bool recursion = true)
    {
        std::vector<std::string> files;
        std::vector<std::string> dirs;
        if(platform::get_dir_info(path, files, dirs, recursion)) {
    ...
    // 替换字符串
    inline static std::string replace_str(std::string str, const std::string &from,
                                          const std::string &to)
    {
        size_t pos = 0;
...
public:
    ...
    // 读取所有行
    std::vector<std::string> read_line_all(
        std::function<bool(const std::string &)> filtrate = nullptr)
    {
    ...
    {
        size_t bnow = tellp();
        std::string sbreak = platform::file_break();
...
public:
    // 退出函数
    std::string operator()(std::string str) { return _str + str; }
