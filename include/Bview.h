...
public:
    ...
    Bview(const Bview &d, uint64 pos, uint64 len = 0) : Bview(d.data(), d.size(), pos, len) {}

    Bview(cstr d, uint64 pos = 0, uint64 len = 0) : Bview(d.c_str(), d.size(), pos, len) {}

    Bview(cchp d, uint64 pos = 0, uint64 len = 0) : Bview(d, strlen(d), pos, len) {}

    Bview(cchp d, uint64 size, uint64 pos, uint64 len)
    {
        _data = d + pos;
        if(len == 0) {
            _size = size - pos;
    ...
    bool operator==(const Bview &d) const
    {
        if(_size != d._size) {
            return false;
        }
    ...
    // 移动头部指针并缩减长度
    inline Bview &move_head(int32 pos)
    {
        if(_data) {
            _data += pos;
            _size -= pos;
    ...
    // 调整尾部长度
    inline Bview &move_tail(int32 pos)
    {
        if(_data) {
            _size += pos;
        }
    ...
    inline uint64 find(Bview d, uint64 pos) const { return find(d.data(), pos, d.size()); }
    ...
    inline uint64 rfind(Bview d, uint64 pos) const { return rfind(d.data(), pos, d.size()); }
